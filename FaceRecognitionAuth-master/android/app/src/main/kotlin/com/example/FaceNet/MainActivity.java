...
import com.example.FaceNet.ZKUSBManager.ZKUSBManager;
import com.example.FaceNet.ZKUSBManager.ZKUSBManagerListener;
import com.example.FaceNet.util.PermissionUtils;
...
public class MainActivity extends FlutterActivity {
    ...
    private static final String CHANNEL = "Emprinte";
        ...
        new MethodChannel(flutterEngine.getDartExecutor().getBinaryMessenger(), CHANNEL).setMethodCallHandler(
                new MethodChannel.MethodCallHandler() {
                    ...
                    public void onMethodCall(@NonNull MethodCall call, @NonNull MethodChannel.Result result) {
                        if (call.method.equals("onBnStart")) {
                            String response = onBnStart();
                            result.success(response);
                        }   else if (call.method.equals("onBnRegister")) {
                        String strUidp = call.argument("strUidp");
                        String response = onBnRegister(strUidp);
                        result.success(response);
                    }
...
public void init(){
    ...
    public String onBnRegister(String strUidp)
    {
        if (bStarted) {
            strUidp = editText.getText().toString();
            if (null == strUid || strUidp.isEmpty()) {
        //        textView.setText("Please input your user id");
                bRegister = false;
                return "Please input your user id";
            }
            if (dbManager.isUserExited(strUidp)) {
                bRegister = false;
          //      textView.setText("The user[" + strUidp + "] had registered!");
                return "The user[" + strUidp + "] had registered!";
            }
            bRegister = true;
            enroll_index = 0;
           ...
           // textView.setText("Please start capture first");
            return "Please start capture first";
        }
    }
